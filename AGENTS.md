...
**vitr** = this template. **ClassClarus** = sample domain (classes, roles, join codes). Keep platform patterns; replace classroom nouns unless `--keep-classroom`.

Clone: `bun run post-clone` → [`CLONE_CHECKLIST.md`](./CLONE_CHECKLIST.md). Keep [`.cursor/rules/`](./.cursor/rules/). Use [`.cursor/skills/`](./.cursor/skills/) for InstantDB / Polar / shadcn.
