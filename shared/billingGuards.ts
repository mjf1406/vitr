...
import { AppError } from "./polarErrors.ts";
import { POLAR_ENV } from "./polarEnv.ts";
...
export function assertConfiguredProduct(productId: string): void {
  ...
  if (!allowed.has(productId)) {
    throw new AppError("INVALID_PRODUCT", "Unknown product");
  }
}
 ...
 * Allows the brand app origin, or localhost / 127.0.0.1 for local Instant + Vite.
 */
