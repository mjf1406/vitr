...
/** Env — set `true` only on the production deployment. */
export function isUsageTrackingEnabled(): boolean {
  return process.env.USAGE_TRACKING_ENABLED === "true" && !isSelfHosted();
}

/**
 * Env — set `true` on a **dev** deployment to render canned Free-card
...
/** Inclusive lower / exclusive upper bounds for time ranges. */
...
export function usageCounterBucket(parts: Array<string>): string {
  return parts.join(":");
}
