...
/** Pull Polar's stable error code from SDK / wrapped failures. */
...
export class AppError extends Error {
  ...
  constructor(code: string, message: string, status = 400, correlation?: string) {
    super(message);
    this.name = "AppError";
    this.code = code;
    this.status = status;
...
/**
 * Convert Polar / unexpected billing failures into a stable AppError.
...
export function throwBillingError(
  ...
  throw new AppError(
    code,
    safeDetail && safeDetail.length < 160 ? safeDetail : fallbackMessage,
    400,
    id,
  );
}
